...
static BOOL     g_cUsePromiscuousMode = FALSE; // can only be set before starting to capture packet 

/**
 * Toggle g_cUsePromiscuousMode. Will make pcap_open_live use PromiscuousMode if possible
 */
...
/* Set g_cCapturePackets to FALSE */
void PausePacketCapture();

/* Set g_cCapturePackets back to TRUE. */
void ResumePacketCapture();

/* Reset g_cPacketCount back to 1. */
...
/**
 * Dump packets in 'packetArray' to a PCAP file specified in 'filePath'.
 * @param  packetArray A list of valid Packet* structs to save information about to the file.
 * @param  numberOfPackets The number of packets saved in 'packetArray'.
 * @param  filePath The path of the PCAP file to save inforamtion in.
 * @return BOOL TRUE if the operation was successful, FALSE otherwise.
 */
BOOL DumpPacketsToFile(struct Packet** packetArray, int numberOfPackets, const char* filePath);

/**
 * Load packets from a PCAP file. Parse the headers and give it ot the frontend.
 * The same process as if we were reading from the internet.
 * @param  filePath The path of the PCAP file to read from.
 * @return BOOL TRUE if the operation was successful, FALSE otherwise.
 */
